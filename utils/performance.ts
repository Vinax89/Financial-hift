/**
 * @fileoverview Performance optimization utilities
 * @description Utilities for React memoization, profiling, and performance monitoring
 */

import { memo, useMemo, useCallback, useRef, useEffect } from 'react';

/**
 * Creates a memoized component with custom comparison
 * @param {React.Component} Component - Component to memoize
 * @param {Function} arePropsEqual - Custom props comparison function
 * @returns {React.Component} Memoized component
 * 
 * @example
 * const MemoizedCard = memoize(Card, (prevProps, nextProps) => 
 *   prevProps.id === nextProps.id && prevProps.data === nextProps.data
 * );
 */
export function memoize(Component, arePropsEqual) {
  return memo(Component, arePropsEqual);
}

/**
 * Creates a shallow comparison memoized component
 * Only re-renders if props reference changes (not deep equality)
 * @param {React.Component} Component - Component to memoize
 * @returns {React.Component} Memoized component
 * 
 * @example
 * export default shallowMemo(ExpensiveComponent);
 */
export function shallowMemo(Component) {
  return memo(Component);
}

/**
 * Memoizes expensive calculations
 * @param {Function} calculate - Calculation function
 * @param {Array} dependencies - Dependency array
 * @returns {*} Memoized value
 * 
 * @example
 * const total = useMemoizedCalc(() => 
 *   transactions.reduce((sum, t) => sum + t.amount, 0),
 *   [transactions]
 * );
 */
export function useMemoizedCalc(calculate, dependencies) {
  return useMemo(calculate, dependencies);
}

/**
 * Memoizes callback functions
 * Prevents child re-renders when passing callbacks
 * @param {Function} callback - Callback function
 * @param {Array} dependencies - Dependency array
 * @returns {Function} Memoized callback
 * 
 * @example
 * const handleClick = useMemoizedCallback(() => {
 *   onClick(id);
 * }, [id, onClick]);
 */
export function useMemoizedCallback(callback, dependencies) {
  return useCallback(callback, dependencies);
}

/**
 * Tracks component render count (development only)
 * @param {string} componentName - Name of component to track
 * @returns {number} Render count
 * 
 * @example
 * const renderCount = useRenderCount('ExpensiveComponent');
 * console.log('Rendered', renderCount, 'times');
 */
export function useRenderCount(componentName) {
  const renderCount = useRef(0);
  
  useEffect(() => {
    renderCount.current++;
    if (import.meta.env.DEV) {
      console.log(`[Performance] ${componentName} rendered ${renderCount.current} times`);
    }
  });
  
  return renderCount.current;
}

/**
 * Measures component render time (development only)
 * @param {string} componentName - Name of component to measure
 * 
 * @example
 * useRenderTime('ExpensiveComponent');
 */
export function useRenderTime(componentName) {
  const startTime = useRef(performance.now());
  
  useEffect(() => {
    const endTime = performance.now();
    const renderTime = endTime - startTime.current;
    
    if (import.meta.env.DEV && renderTime > 16) { // >16ms = might cause frame drop
      console.warn(`[Performance] ${componentName} render took ${renderTime.toFixed(2)}ms`);
    }
    
    startTime.current = performance.now();
  });
}

/**
 * Debounces a value to prevent excessive updates
 * @param {*} value - Value to debounce
 * @param {number} delay - Delay in milliseconds
 * @returns {*} Debounced value
 * 
 * @example
 * const debouncedSearchTerm = useDebouncedValue(searchTerm, 300);
 */
export function useDebouncedValue(value, delay = 500) {
  const [debouncedValue, setDebouncedValue] = useState(value);
  
  useEffect(() => {
    const handler = setTimeout(() => {
      setDebouncedValue(value);
    }, delay);
    
    return () => clearTimeout(handler);
  }, [value, delay]);
  
  return debouncedValue;
}

/**
 * Throttles a function to limit execution rate
 * @param {Function} callback - Function to throttle
 * @param {number} limit - Minimum time between executions (ms)
 * @returns {Function} Throttled function
 * 
 * @example
 * const handleScroll = useThrottle(() => {
 *   // expensive scroll handler
 * }, 100);
 */
export function useThrottle(callback, limit = 200) {
  const inThrottle = useRef(false);
  
  return useCallback((...args) => {
    if (!inThrottle.current) {
      callback(...args);
      inThrottle.current = true;
      
      setTimeout(() => {
        inThrottle.current = false;
      }, limit);
    }
  }, [callback, limit]);
}

/**
 * Memoizes array/object calculations to prevent reference changes
 * @param {Array|Object} data - Data to stabilize
 * @param {Function} comparator - Optional custom comparison
 * @returns {Array|Object} Stabilized reference
 * 
 * @example
 * const categories = useStableReference(
 *   transactions.map(t => t.category)
 * );
 */
export function useStableReference(data, comparator) {
  const dataRef = useRef(data);
  
  useMemo(() => {
    if (comparator) {
      if (!comparator(dataRef.current, data)) {
        dataRef.current = data;
      }
    } else {
      // Simple JSON comparison (works for simple objects/arrays)
      if (JSON.stringify(dataRef.current) !== JSON.stringify(data)) {
        dataRef.current = data;
      }
    }
  }, [data, comparator]);
  
  return dataRef.current;
}

/**
 * Prevents component from re-rendering more than once per frame
 * Useful for high-frequency updates (scroll, resize, etc.)
 * @param {*} value - Value to update
 * @returns {*} Frame-throttled value
 * 
 * @example
 * const scrollY = useFrameThrottle(currentScrollY);
 */
export function useFrameThrottle(value) {
  const [throttledValue, setThrottledValue] = useState(value);
  const frameRef = useRef(null);
  
  useEffect(() => {
    if (frameRef.current) {
      cancelAnimationFrame(frameRef.current);
    }
    
    frameRef.current = requestAnimationFrame(() => {
      setThrottledValue(value);
    });
    
    return () => {
      if (frameRef.current) {
        cancelAnimationFrame(frameRef.current);
      }
    };
  }, [value]);
  
  return throttledValue;
}

/**
 * Lazy initializes expensive state
 * @param {Function} initializer - State initialization function
 * @returns {*} Initialized state
 * 
 * @example
 * const [data, setData] = useState(() => 
 *   useLazyState(() => calculateExpensiveInitialData())
 * );
 */
export function useLazyState(initializer) {
  return useMemo(initializer, []);
}

/**
 * Performance profiling decorator
 * Wraps a function to measure execution time
 * @param {Function} fn - Function to profile
 * @param {string} label - Label for profiling output
 * @returns {Function} Profiled function
 * 
 * @example
 * const calculate = profileFunction(expensiveCalc, 'Calculation');
 */
export function profileFunction(fn, label) {
  return (...args) => {
    if (!import.meta.env.DEV) {
      return fn(...args);
    }
    
    const start = performance.now();
    const result = fn(...args);
    const end = performance.now();
    
    console.log(`[Profile] ${label}: ${(end - start).toFixed(2)}ms`);
    
    return result;
  };
}

/**
 * Batch multiple state updates to prevent excessive re-renders
 * @param {Function} callback - Function containing state updates
 * 
 * @example
 * batchUpdates(() => {
 *   setState1(value1);
 *   setState2(value2);
 *   setState3(value3);
 * });
 */
export function batchUpdates(callback) {
  // React 18+ automatically batches, but explicitly batch for older versions
  if (typeof queueMicrotask !== 'undefined') {
    queueMicrotask(callback);
  } else {
    Promise.resolve().then(callback);
  }
}

/**
 * Memoizes data filtering/transformation operations
 * @param {Array} data - Data array
 * @param {Function} filterFn - Filter function
 * @param {Function} transformFn - Transform function
 * @returns {Array} Filtered and transformed data
 * 
 * @example
 * const visibleTransactions = useMemoizedFilter(
 *   transactions,
 *   t => t.amount > 0,
 *   t => ({ ...t, formatted: formatCurrency(t.amount) })
 * );
 */
export function useMemoizedFilter(data, filterFn, transformFn) {
  return useMemo(() => {
    const filtered = data.filter(filterFn);
    return transformFn ? filtered.map(transformFn) : filtered;
  }, [data, filterFn, transformFn]);
}

// Missing import
import { useState } from 'react';

export default {
  memoize,
  shallowMemo,
  useMemoizedCalc,
  useMemoizedCallback,
  useRenderCount,
  useRenderTime,
  useDebouncedValue,
  useThrottle,
  useStableReference,
  useFrameThrottle,
  useLazyState,
  profileFunction,
  batchUpdates,
  useMemoizedFilter
};
