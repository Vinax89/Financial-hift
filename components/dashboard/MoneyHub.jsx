...
export default function MoneyHub({ metrics, transactions, shifts, goals, debts, bills, refreshData }) {
    const monthlyData = useMemo(() => {
        ...
        return { income, expenses, net: income - expenses, transactions: monthlyTransactions.length };
    }, [transactions]);
                                ...
                                <div className="space-y-2">
                                    {upcomingShifts.map((shift) => (
                                        <div key={shift.id} className="flex justify-between items-center p-2 bg-slate-50 rounded-lg">
                                            ...
                                            <Badge variant="secondary">{shift.scheduled_hours}h</Badge>
                                        </div>
                                    ))}
                                </div>
                            ) : (
                            ...
                            goalProgress.map((goal) => (
                                <div key={goal.id} className="space-y-2">
                                    <div className="flex justify-between items-center">
                                        ...
                                        <Badge 
                                            variant={goal.progress >= 100 ? "success" : goal.progress >= 50 ? "default" : "secondary"}
                                        >
